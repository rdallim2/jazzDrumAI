...
class DrumPattern {
    ...
    async playNote(note, velocity = 0.7, duration = 0.1) {
        if (this.synth) {
            await this.synth.play(note, undefined, {
                gain: velocity,
                duration: duration
            });
        }
    ...
    async swingPattern(timePerBeat, tripSpacing) {
        ...
        // First beat
        this.playNote(DRUMS.BASS_DRUM, 0.15);
        this.playNote(DRUMS.RIDE_CYMBAL, 0.9);
        
        // Second beat with swing
        setTimeout(() => {
            this.playNote(DRUMS.BASS_DRUM, 0.15);
            this.playNote(DRUMS.RIDE_CYMBAL, 1.05);
            this.playNote(DRUMS.HI_HAT_CLOSED, 0.7);
        }, timePerBeat * 1000);
        
        // Third beat with swing
        setTimeout(() => {
            this.playNote(DRUMS.RIDE_CYMBAL, 0.95);
        }, (timePerBeat + (timePerBeat * tripSpacing)) * 1000);
    }

    // Additional pattern methods will be implemented here
    // Each pattern will follow the same structure as swingPattern
}
...
export { DrumPattern, chooseNextPattern, getTripSpacing, DRUMS };
