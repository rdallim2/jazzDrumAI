...
class DrumMachineGUI(BoxLayout):
    ...
    def initialize_audio(self):
        try:
            ...
            # Set up piano on channel 0
            fs.sfont_select(0, piano_sfid)
            fs.program_select(0, piano_sfid, 0, 0)  # Piano on channel 0
            fs.cc(0, 7, 50)  # Set piano volume
            
            # Set up bass on channel 9
            fs.sfont_select(9, piano_sfid)  # Select soundfont for bass channel
            fs.program_select(9, piano_sfid, 0, 32)  # Bass on channel 9
            fs.cc(9, 7, 60)  # Set bass volume

            # Load SoundFont for drum sounds and set up on channel 10
            drum_sfid = fs.sfload("drums_for_ai_v10.sf2")
            if drum_sfid == -1:
                raise Exception("Failed to load drum SoundFont")
            
            fs.sfont_select(10, drum_sfid)  # Select drum soundfont for channel 10
            fs.program_select(10, drum_sfid, 128, 0)  # Set up drums on channel 10, bank 128 (GM drums), preset 0
            fs.cc(10, 7, 100)  # Set drum volume
            
            print("Audio initialization complete - Piano on ch 0, Bass on ch 9, Drums on ch 10")
    ...
    def run_piano_wrapper(self, time_per_beat, tempo, piano_channel=0):
        try:
            ...
            print(f"Error in piano thread: {e}")  # Also fixed error message to say piano instead of bass
