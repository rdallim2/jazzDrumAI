...
def play_bar(fs, time_per_beat, channel=9):
    ...
    while True and not stop_event.is_set():  # Loop indefinitely for background bass
        ...
        # Bars 1-4: C7 | C7 | C7 | C7
        # Bars 5-8: F7 | F7 | C7 | C7
        # Bars 9-12: G7 | F7 | C7 | G7
        if current_bar < 4:
            chord = blues_progression[0]  # I (C7)
        elif current_bar < 6:
            chord = blues_progression[1]  # IV (F7)
        elif current_bar < 8:
            chord = blues_progression[0]  # I (C7)
        elif current_bar == 8:
            chord = blues_progression[2]  # V (G7)
        elif current_bar == 9:
            chord = blues_progression[1]  # IV (F7)
        elif current_bar == 10:
            chord = blues_progression[0]  # I (C7)
        else:
            chord = blues_progression[2]  # V (G7) - Turnaround
