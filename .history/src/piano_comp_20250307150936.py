...
def piano_comp(fs, time_per_beat, tempo, channel=10):
    ...
    bar_count = 1
    total_bars = 12
    expected_bar_duration = time_per_beat * 4  # Each bar should be 4 beats

    trip_spacing = get_trip_spacing(tempo)

    # Get initial chord and voicing
    ...
    print(f"Piano starting on channel {channel} with first voicing: {chord_voicing}")
    print(f"Expected bar duration: {expected_bar_duration:.3f} seconds (tempo={tempo} BPM, time_per_beat={time_per_beat:.3f}s)")
    ...
    while True:
        ...
        print(f"Starting piano phrase {phrase_num} on bar {current_bar} with voicing {chord_voicing}")
        ...
        bar_duration = time.time() - bar_start_time
        timing_error = bar_duration - expected_bar_duration
        print(f"Bar {current_bar} timing - Expected: {expected_bar_duration:.3f}s, Actual: {bar_duration:.3f}s, Error: {timing_error:.3f}s")
        print(f"Phrase {phrase_num} took {phrase_duration:.3f}s")

        time_per_beat -= timing_error / (total_beats_per_bar * total_bars)
        time_per_beat = max(time_per_beat, 0.01) 
