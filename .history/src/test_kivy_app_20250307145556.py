...
class DrumMachineGUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.start_time = 0  # Shared time reference
        self.current_beat = 0  # Current beat counter
        self.beat_condition = threading.Condition()  # Condition for synchronizing beats
    ...
    def wait_for_next_beat(self, time_per_beat):
        with self.beat_condition:
            target_time = self.start_time + (self.current_beat * time_per_beat)
    ...
    def run_drums_wrapper(self, time_per_beat, tempo, players):
        try:
            ...
            while not self.stop_event.is_set():
                ...
                if comp_choice == 'n':
                    self.wait_for_next_beat(time_per_beat)  # Synchronize to next beat
