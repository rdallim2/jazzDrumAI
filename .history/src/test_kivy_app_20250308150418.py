...
class DrumMachineGUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # Set background color with a simple gradient-like effect
        with self.canvas.before:
            # Create a two-color background (simpler than texture-based gradient)
            # Top part (darker red)
            Color(0.9, 0.2, 0.2, 1)  # Red at top
            self.bg_rect_top = Rectangle(pos=self.pos, 
                                     size=(self.size[0], self.size[1] * 0.5))
            
            # Bottom part (orange)
            Color(0.9, 0.5, 0.1, 1)  # Orange at bottom
            self.bg_rect_bottom = Rectangle(pos=(self.pos[0], self.pos[1] + self.size[1] * 0.5), 
                                        size=(self.size[0], self.size[1] * 0.5))
    ...
    def _update_rect(self, instance, value):
        ...
        self.bg_rect_top.size = (instance.size[0], instance.size[1] * 0.5)
        
        self.bg_rect_bottom.pos = (instance.pos[0], instance.pos[1] + instance.size[1] * 0.5)
        self.bg_rect_bottom.size = (instance.size[0], instance.size[1] * 0.5)
