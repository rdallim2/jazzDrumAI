...
from kivy.graphics import Color, Rectangle, Ellipse, Rectangle, InstructionGroup
...
class DrumMachineGUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        with self.canvas.before:
            Color(1, 1, 1, 1)  # White background for the gradient container
            self.bg_rect = Rectangle(pos=self.pos, size=self.size)
            
            # Add gradient texture
            vertical_gradient = Gradient.vertical(
                (0.9, 0.2, 0.2, 1),    # Red at top
                (0.9, 0.5, 0.1, 1)     # Orange at bottom
            )
            Color(1, 1, 1, 1)  # Full opacity for gradient
            self.gradient_rect = Rectangle(texture=vertical_gradient, pos=self.pos, size=self.size)
        ...
        title_label = Label(
            ...
            outline_color=(0.3, 0.1, 0.1),  # Dark red outline
            outline_width=2,
            size_hint_y=None,
            height=80
        )
        ...
        # Create a container for the controls with a semi-transparent background
        ...
        with controls_container.canvas.before:
            ...
            self.controls_bg = RoundedRectangle(pos=controls_container.pos, size=controls_container.size, radius=[15, 15, 15, 15])
        ...
        tempo_icon = Label(
            ...
            size_hint_x=None,
            width=30,
            color=(1, 0.9, 0.6, 1)  # Light golden color
        )
        ...
        self.tempo_slider = Slider(
            ...
            step=1,
            cursor_size=(25, 25),
            cursor_image='',  # Custom image could be added here
            background_horizontal='',  # Custom background could be added
            size_hint_x=0.7
        )
        ...
        self.mode_spinner = Spinner(
            ...
            size_hint_y=None,
            height=40,
            background_color=(0.9, 0.4, 0.3, 0.7),  # Semi-transparent reddish
            background_normal='',
            color=(1, 1, 1, 1),
        ...
        with status_layout.canvas.before:
            ...
            self.status_bg = RoundedRectangle(pos=status_layout.pos, size=status_layout.size, radius=[10, 10, 10, 10])
...
class Gradient:
    ...
    def horizontal(*args):
        ...
        buf = bytes([int(v * 255) for arg in args for v in arg])
...
class RoundedRectangle(InstructionGroup):
    ...
    def _draw(self):
        ...
        # Top-left corner
        self.add(Ellipse(pos=self.pos, size=[self.radius[0] * 2] * 2, angle_start=180, angle_end=270))
        
        # Top-right corner
        self.add(Ellipse(pos=[self.pos[0] + self.size[0] - self.radius[1] * 2, self.pos[1]], 
                          size=[self.radius[1] * 2] * 2, angle_start=270, angle_end=360))
        
        # Bottom-right corner
        self.add(Ellipse(pos=[self.pos[0] + self.size[0] - self.radius[2] * 2, 
                              self.pos[1] + self.size[1] - self.radius[2] * 2], 
                          size=[self.radius[2] * 2] * 2, angle_start=0, angle_end=90))
        
        # Bottom-left corner
        self.add(Ellipse(pos=[self.pos[0], self.pos[1] + self.size[1] - self.radius[3] * 2], 
                          size=[self.radius[3] * 2] * 2, angle_start=90, angle_end=180))
        
        # Draw four rectangles to fill the space between the corners
        # Top rectangle
        self.add(Rectangle(pos=[self.pos[0] + self.radius[0], self.pos[1]], 
                           size=[self.size[0] - self.radius[0] - self.radius[1], self.radius[0]]))
        
        # Right rectangle
        self.add(Rectangle(pos=[self.pos[0] + self.size[0] - self.radius[1], self.pos[1] + self.radius[1]], 
                           size=[self.radius[1], self.size[1] - self.radius[1] - self.radius[2]]))
        
        # Bottom rectangle
        self.add(Rectangle(pos=[self.pos[0] + self.radius[3], self.pos[1] + self.size[1] - self.radius[2]], 
                           size=[self.size[0] - self.radius[2] - self.radius[3], self.radius[2]]))
        
        # Left rectangle
        self.add(Rectangle(pos=[self.pos[0], self.pos[1] + self.radius[0]], 
                           size=[self.radius[0], self.size[1] - self.radius[0] - self.radius[3]]))
        
        # Center rectangle
        self.add(Rectangle(pos=[self.pos[0] + self.radius[0], self.pos[1] + self.radius[0]], 
                           size=[self.size[0] - self.radius[0] - self.radius[1], 
                                self.size[1] - self.radius[0] - self.radius[2]]))
...
class DrumMachineApp(App):
    def build(self):
        ...
        Window.clearcolor = (0.9, 0.2, 0.2, 1)  # Red fallback color
