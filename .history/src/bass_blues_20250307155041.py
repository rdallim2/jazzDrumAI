import random
import time
from sync_utils_new import BarSync
...
def walking_bass_line(fs, time_per_beat, channel=9):
 ...
 # Walking bass patterns for each chord
    ...
    while True:
        sync.wait_for_bar()  # Wait for the next bar to start
        current_bar = bar_count % total_bars
        ...
        print(f"Bass bar {bar_count}: elapsed={elapsed:.3f}s, expected={expected:.3f}s")