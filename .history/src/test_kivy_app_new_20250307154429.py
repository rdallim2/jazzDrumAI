...
class DrumMachineGUI(BoxLayout):
    ...
    def initialize_audio(self):
        try:
            ...
            fs.program_select(0, piano_sfid, 0, 0)  # Piano
            fs.cc(0, 7, 45)  # Piano volume
            
            # Set up bass on channel 1
            fs.sfont_select(1, piano_sfid)
            fs.program_select(1, piano_sfid, 0, 32)  # Bass
            fs.cc(1, 7, 60)  # Bass volume

            # Load and set up drums on channel 9 (standard MIDI drum channel)
            drum_sfid = fs.sfload("drums_for_ai_v10.sf2")
            if drum_sfid == -1:
                raise Exception("Failed to load drum SoundFont")
            
            fs.sfont_select(9, drum_sfid)
            fs.program_select(9, drum_sfid, 128, 0)  # Bank 128 for GM drums
            fs.cc(9, 7, 100)  # Drum volume
            
            print("Audio initialization complete - Piano ch 0, Bass ch 1, Drums ch 9")
