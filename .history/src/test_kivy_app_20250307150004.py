...
class DrumMachineGUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # Timing calibration factors for each thread
        self.timing_factors = {
            'drums': 1.0,
            'bass': 1.0,
            'piano': 1.0
    ...
    def initialize_audio(self):
        try:
            ...
            fs.cc(10, 7, 50)  # Set piano volume
    ...
    def calibrate_timing(self, thread_name, expected_duration, actual_duration):
        """Adjust timing factor based on actual vs expected duration"""
        if actual_duration > 0:  # Avoid division by zero
            new_factor = expected_duration / actual_duration
            # Use moving average to smooth out adjustments
            self.timing_factors[thread_name] = (self.timing_factors[thread_name] * 0.8) + (new_factor * 0.2)
            print(f"{thread_name} timing factor: {self.timing_factors[thread_name]}")
    ...
    def run_drums_wrapper(self, time_per_beat, tempo, players):
        try:
            ...
            while not self.stop_event.is_set():
                ...
                if comp_choice == 'n':
                    swing_pattern(fs, time_per_beat * self.timing_factors['drums'], trip_spacing)
                else:
                    if curr_density == '8':
                        if curr_vol == 'l':
                            ...
                            random.choice(eigth_phrases)(fs, time_per_beat * self.timing_factors['drums'], trip_spacing)
                        elif curr_vol == 'm':
                            s8_med_phrases = [s8_s_one, s8_s_two, s8_crash_one, s8_crash_two, s8_b_one]
                            random.choice(s8_med_phrases)(fs, time_per_beat * self.timing_factors['drums'], trip_spacing)
                        else:
                            s8_high_phrases = [s8_crash_one, s8_crash_two]
                            random.choice(s8_high_phrases)(fs, time_per_beat * self.timing_factors['drums'], trip_spacing)

                # Measure actual duration and calibrate
                actual_duration = time.time() - start_time
                expected_duration = time_per_beat * 4  # One bar
                self.calibrate_timing('drums', expected_duration, actual_duration)
    ...
    def run_bass_wrapper(self, time_per_beat, bass_channel=9):
        try:
            while not self.stop_event.is_set():
                start_time = time.time()
                walking_bass_line(fs, time_per_beat * self.timing_factors['bass'], bass_channel)
                # Measure actual duration and calibrate
                actual_duration = time.time() - start_time
                expected_duration = time_per_beat * 4  # One bar
                self.calibrate_timing('bass', expected_duration, actual_duration)
        except Exception as e:
    ...
    def run_piano_wrapper(self, time_per_beat, tempo, piano_channel=10):
        try:
            while not self.stop_event.is_set():
                start_time = time.time()
                piano_comp(fs, time_per_beat * self.timing_factors['piano'], tempo, piano_channel)
                # Measure actual duration and calibrate
                actual_duration = time.time() - start_time
                expected_duration = time_per_beat * 4  # One bar
                self.calibrate_timing('piano', expected_duration, actual_duration)
        except Exception as e:
