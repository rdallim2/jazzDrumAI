...
from sync_utils_new import sync
from new_app import *
...
class DrumMachineGUI(BoxLayout):
        ...
        try:
            ...
            while not self.stop_event.is_set():
                ...
                if comp_choice == 'n':  
                    ...
                    if curr_density == '8':
                        if curr_vol == 'l':
                            eigth_phrases = [s8_s_one, s8_s_two, s8_s_three, s8_s_four]
                ...
                # Signal bar completion and wait for others
                sync.end_bar()
    ...
    def run_bass_wrapper(self, time_per_beat, bass_channel=1):
        try:
            while not self.stop_event.is_set():
                # Wait for all instruments to be ready
                sync.wait_for_bar()
    ...
    def run_piano_wrapper(self, time_per_beat, tempo, piano_channel=0):
        try:
            while not self.stop_event.is_set():
                # Wait for all instruments to be ready
                sync.wait_for_bar()
    ...
    def start_performance(self, instance):
        ...
        # Convert mode to player number and register instruments
        sync.total_instruments = 0  # Reset instrument count
        if 'Drums' in mode:
            sync.register_instrument()
        if 'Bass' in mode and 'play' not in mode:
            sync.register_instrument()
        if 'Piano' in mode and 'play' not in mode:
            sync.register_instrument()

        time_per_beat = 60 / tempo
        sync.set_tempo(tempo)
        sync.start()
        ...
        try:
            ...
            if 'Drums' in mode:
                drum_thread = threading.Thread(
                    ...
                    args=(time_per_beat, tempo, sync.total_instruments)
                )
            ...
            if 'Bass' in mode and 'play' not in mode:
                bass_thread = threading.Thread(
                    ...
                    args=(time_per_beat, 1)  # Channel 1 for bass
                )
                bass_thread.daemon = True
                bass_thread.start()
                self.running_threads.append(bass_thread)
            ...
            if 'Piano' in mode:
                if 'play' in mode:
                    ...
                    midi_thread.start()
                    self.running_threads.append(midi_thread)
                else:
                    piano_thread = threading.Thread(
                        target=self.run_piano_wrapper,
                        args=(time_per_beat, tempo, 0)  # Channel 0 for piano
                    )
                    piano_thread.daemon = True
                    piano_thread.start()
                    self.running_threads.append(piano_thread)
