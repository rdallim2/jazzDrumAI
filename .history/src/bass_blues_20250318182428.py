    ...
    while True and not stop_event.is_set():  # Loop indefinitely for background bass
        ...
        for _ in range(BEATS_PER_BAR):  # Play 4 notes per bar    
            if not stop_event.is_set():
                ...
                if last not in chord:
                    last_note = random.choice(chord)  
                else:
                    last_note = last

                print("Last note index: ", last_note)

                last_note_idx = chord.index(last_note)
                probabilities = note_choice_mat[last_note_idx]
                ...
                next_note_idx = random.choices(range(len(chord)), weights=probabilities, k=1)[0]
                note = chord[next_note_idx]
