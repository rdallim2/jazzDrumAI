    ...
    def handle_midi_input_wrapper(self, tempo):
        ...
        try:
            ...
            if not available_ports:
                ...
                self.status_label.text = "Error: No MIDI devices found"
                return
            
            # Choose a suitable port - try to find a keyboard or default to the first one
            port_index = 0  # Default to first port
            for i, port_name in enumerate(available_ports):
                if 'keyboard' in port_name.lower() or 'midi' in port_name.lower():
                    port_index = i
                    break
            
            port_name = available_ports[port_index]
            print(f"Using MIDI input port: {port_name} (index {port_index})")
            
            # Open the port with timeout-based handling to allow checking stop_event
            with mido.open_input(port_name) as port:
                print("Listening for Keyboard MIDI input... Press Stop to exit.")
                
                while not stop_event.is_set():
                    # Use receive with timeout to allow checking stop_event
                    msg = port.receive(block=True, timeout=0.1)
                    
                    if msg is None:
                        # No message received (timeout), continue loop
                        continue
                    ...
                    # Don't call analyze_density too frequently
                    if random.random() < 0.1:  # Only analyze about 10% of messages
                        analyze_density(tempo)
                        
        except IndexError as e:
            print(f"Error with MIDI port selection: {e}")
            self.status_label.text = "Error: MIDI port not available"
        except Exception as e:
            print(f"Error in MIDI handling: {e}")
            self.status_label.text = f"MIDI error: {str(e)}"
        finally:
