...
def init_phrase(channel, fs, time_per_beat, trip_spacing, chord):
    ...
    print("should've heard something in the piano")
    time.sleep(time_per_beat)
    for note in chord:
        fs.noteoff(channel, note)  # Turn off previous notes
        fs.noteon(channel, note, 70)
    time.sleep(time_per_beat)
    for note in chord:  # Turn off final notes
...
def phrase_one(channel, fs, time_per_beat, trip_spacing, chord):
    ...
    for note in chord:  # Turn off notes
        fs.noteoff(channel, note)
    time.sleep(time_per_beat)
...
def phrase_two(channel, fs, time_per_beat, trip_spacing, chord):
    ...
    for note in chord:
        fs.noteon(channel, note, 70)
    time.sleep(time_per_beat)
    for note in chord:  # Turn off previous notes
        fs.noteoff(channel, note)
        fs.noteon(channel, note, 70)
    time.sleep(time_per_beat)
    for note in chord:  # Turn off final notes
