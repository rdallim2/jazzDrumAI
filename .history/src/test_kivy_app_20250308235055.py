    ...
    def start_performance(self, instance):
        ...
        try:
            ...
            if players == 1:
                ...
                # Add MIDI input thread to handle keyboard input
                # This prevents crashes when user presses MIDI keys in this mode
                midi_thread = threading.Thread(
                    target=self.handle_midi_input_wrapper,
                    args=(tempo,)
                )
                ...
                self.running_threads.extend([drum_thread, bass_thread, midi_thread])
