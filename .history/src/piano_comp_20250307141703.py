...
def piano_comp(fs, time_per_beat, tempo, channel=10):
    ...
    chord = piano_chords["C7"]  
    chord_voicing = random.choice(chord)
    ...
    while True:
        ...
        phrase = random.randint(1, 3)  # Choose a random phrase between 1 and 3
        print(f"Playing piano phrase {phrase} on bar {current_bar}")
        
        if phrase == 1:
            phrase_one(channel, fs, time_per_beat, trip_spacing, chord)
        elif phrase == 2:
            phrase_two(channel, fs, time_per_beat, trip_spacing, chord)
        elif phrase == 3:
            phrase_three(channel, fs, time_per_beat, trip_spacing, chord)
