...
class DrumMachineGUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # Create a more visual lead sheet style display
        ...
        notation_label = Label(
            text='Standard 12-Bar Blues (4/4)',
            font_name='Roboto',
            font_size='16sp',
            italic=True,
            color=(0.8, 0.8, 0.8, 1)
        ...
        line1_measures = list(reversed(line1.children))
        line2_measures = list(reversed(line2.children))
        line3_measures = list(reversed(line3.children))
        
        # Add all measures in the correct order
        self.measure_buttons.extend(line1_measures)  # First row: 0-3
        self.measure_buttons.extend(line2_measures)  # Second row: 4-7
        self.measure_buttons.extend(line3_measures)  # Third row: 8-11
        
        # Add a bar number indicator for each measure
        for i, measure in enumerate(self.measure_buttons):
            bar_label = Label(
                text=f"Bar {i+1}",
                font_name='Roboto',
                font_size='12sp',
            ...
            # Add the bar number above the measure
            label_box = BoxLayout(orientation='vertical', size_hint_y=None, height=20)
            label_box.add_widget(bar_label)
            
            # If we're at the beginning of a line, add extra space
            if i % 4 == 0:
                lead_sheet.add_widget(BoxLayout(size_hint_y=None, height=5))  # Small spacer
        
        # Add the lead sheet to the controls container
        controls_container.add_widget(lead_sheet)
        
        # Keep the original chord display as a reference
        ...
        self.chord_buttons = [
            Button(text='C7', font_name='Roboto', font_size='18sp', bold=True, color=(0.9, 0.9, 0.9, 1)),
            Button(text='C7', font_name='Roboto', font_size='18sp', bold=True, color=(0.9, 0.9, 0.9, 1)),
            Button(text='C7', font_name='Roboto', font_size='18sp', bold=True, color=(0.9, 0.9, 0.9, 1)),
            Button(text='C7', font_name='Roboto', font_size='18sp', bold=True, color=(0.9, 0.9, 0.9, 1))
        ]
        self.chord_display.add_widget(self.chord_buttons[0])
        self.chord_display.add_widget(self.chord_buttons[1])
        self.chord_display.add_widget(self.chord_buttons[2])
        self.chord_display.add_widget(self.chord_buttons[3])
    ...
    def update_chord_display(self, bar_index):
        ...
        def update_ui(dt):
            ...
            # Also reset all measure buttons in the lead sheet
            ...
            # Highlight the current bar in both displays
            if 0 <= current_index < len(self.chord_buttons):
                # Highlight in the chord buttons display
