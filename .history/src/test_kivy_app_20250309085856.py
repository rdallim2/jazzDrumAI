...
class DrumMachineGUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # Add 12-bar blues chord display (with 4 measures per line)
        chord_display_title = Label(
            text='12-Bar Blues in C',
            font_name='Roboto',
            font_size='18sp',
            bold=True,
        ...
        controls_container.add_widget(chord_display_title)
        
        # Create the chord display layout
        self.chord_display = BoxLayout(
            orientation='vertical',
            size_hint_y=None,
        ...
        # First line (bars 1-4): C7 | C7 | C7 | C7
        line1 = BoxLayout(size_hint_y=None, height=40)  # Increased height
        for i in range(4):
            bar = Button(
                text=f"C7",
                background_color=(0.2, 0.2, 0.25, 1),  # Dark blue-gray
                background_normal='',
            ...
            line1.add_widget(bar)
        self.chord_display.add_widget(line1)
        
        # Second line (bars 5-8): F7 | F7 | C7 | C7
        line2 = BoxLayout(size_hint_y=None, height=40)  # Increased height
        for i in range(4, 8):
            chord = "F7" if i < 6 else "C7"
            bar = Button(
                text=chord,
                background_color=(0.2, 0.2, 0.25, 1),  # Dark blue-gray
                background_normal='',
            ...
            line2.add_widget(bar)
        self.chord_display.add_widget(line2)
        
        # Third line (bars 9-12): G7 | F7 | C7 | G7
        line3 = BoxLayout(size_hint_y=None, height=40)  # Increased height
        chords = ["G7", "F7", "C7", "G7"]
        for i, chord in enumerate(chords):
            ...
            line3.add_widget(bar)
        self.chord_display.add_widget(line3)
        
        # Store references to all chord buttons for highlighting
        self.chord_buttons = []
        # Get all children (Kivy stores children in reverse order)
        line1_buttons = list(reversed(line1.children))
        line2_buttons = list(reversed(line2.children))
        line3_buttons = list(reversed(line3.children))
        
        # Add all buttons in the correct order
        self.chord_buttons.extend(line1_buttons)  # First row: 0-3
        self.chord_buttons.extend(line2_buttons)  # Second row: 4-7
        self.chord_buttons.extend(line3_buttons)  # Third row: 8-11
        
        # Add the chord display to the controls container
        controls_container.add_widget(self.chord_display)
    ...
    def update_chord_display(self, bar_index):
        ...
        def update_ui(dt):
            # Reset all buttons to default color and text color
            for button in self.chord_buttons:
                button.background_color = (0.2, 0.2, 0.25, 1)  # Default dark blue-gray
                button.color = (0.9, 0.9, 0.9, 1)  # Reset to default light gray text
                
            # Calculate current index (0-11)
            current_index = bar_index % 12
            
            # Highlight the current bar
            if 0 <= current_index < len(self.chord_buttons):
                # Make highlighting much more prominent
                self.chord_buttons[current_index].background_color = (0.5, 0.7, 1.0, 1)  # Bright blue highlight
                # Also update the text color for even more emphasis
                self.chord_buttons[current_index].color = (1, 1, 1, 1)  # Bright white text
    ...
    def run_bass_wrapper(self, time_per_beat, bass_channel=9):
        try:
            # Reset the global bar_count in bass_blues
            global bar_count
            bar_count = 0
            self.current_bar = 0
            
            # Register for bar update notifications
            register_bar_update_callback(self.on_bar_changed)
            
            # Initialize instrument_sync event
            instrument_sync.set()
            
            while not stop_event.is_set():
                try:
                    # Play one bar of bass - the callback will update the UI now
                    play_bar(fs, time_per_beat, channel=bass_channel)
                    
                    # Set/clear the sync event
                    instrument_sync.clear()
            ...
            unregister_bar_update_callback(self.on_bar_changed)
            
            # Reset all chord highlighting
            Clock.schedule_once(lambda dt: self.update_chord_display(-1), 0)
    ...
    def stop_performance(self, instance):
        ...
        max_wait_time = 1.0  # Increase timeout for thread joining
            ...
            if fs is not None:
                ...
                for channel in [0, 9, 10]:
                    try:
                        ...
                        print(f"Error clearing notes on channel {channel}: {e}")
                
                # Sleep longer to allow audio to clear
                time.sleep(0.3)
                
                # Do NOT delete fs here as it may be in use by other threads
                # Just silence it for now, will recreate in start_performance
            else:
                print("Warning: FluidSynth instance was None during cleanup")
        except Exception as e:
            print(f"Error during audio cleanup: {e}")
        
        # Reset the chord display highlighting
        self.update_chord_display(-1)
