import time
import random
from time import sleep
import threading
import fluidsynth
import mido
import pygame.midi


fs = fluidsynth.Synth()
fs.start(driver="coreaudio")

# Constants for drum MIDI notes
BASS_DRUM = 36
SNARE_DRUM = 38
HI_HAT_CLOSED = 42
RIDE_CYMBAL = 51
CRASH_CYMBAL = 49

# Drum channel should be 0
DRUM_CHANNEL = 0

# MIDI note for Ride Cymbal (General MIDI standard)
RIDE_CYMBAL_GM = 3
BASS_DRUM_GM = 25
SNARE_DRUM_GM = 37
H_TOM = 29
HI_HAT_CLOSED_GM = 13
CRASH_SIZ_V1 = 7
CRASH_SIZ_V2 = 8
CRASH_SIZ_V3 = 9

def get_trip_spacing(tempo):
    spacing = .5 + (.666 - .5) * ((350 - tempo) / (350 - 250))
    if tempo < 250:
        return .666
    elif 250 <= tempo <= 350:
        print(f"Triplet spacing = {spacing}")
        return spacing
    elif tempo > 350:
        return .5
        
def swing_pattern(fs, time_per_beat, trip_spacing):
    print(f"Playing swing pattern on channel {DRUM_CHANNEL}")
    # First beat
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 80)  # Increased velocity
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * 0.666)  # Swing feel

    # "And" of 1
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 80)  # Increased velocity
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 70)
    sleep(time_per_beat * 0.333)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat)

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 80)   # Increased velocity
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * (1 - trip_spacing))  


#--------------------------------- 8TH PATTERNS ENDING ON SNARE  ------------------------


def s8_s_one(fs, time_per_beat, trip_spacing):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)    
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50)
    sleep(time_per_beat * 1/3) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * (1 - trip_spacing))  

def s8_s_two(fs, time_per_beat, trip_spacing):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)   
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50)
    sleep(time_per_beat * (1 - trip_spacing))  

def s8_s_three(fs, time_per_beat, trip_spacing):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)    
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 35)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50)
    sleep(time_per_beat * (1 - trip_spacing)) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * (1 - trip_spacing)) 

def s8_s_four(fs, time_per_beat, trip_spacing):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)   
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 35)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50)
    sleep(time_per_beat * (1 - trip_spacing))  

def s8_s_five(fs, time_per_beat, trip_spacing):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)    
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 50)
    sleep(time_per_beat * (1 - trip_spacing)) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 50)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50)
    sleep(time_per_beat * (1 - trip_spacing)) 


def s8_s_six(fs, time_per_beat, trip_spacing):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)    
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50)
    sleep(time_per_beat * (1 - trip_spacing))

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * (1 - trip_spacing)) 


def s8_s_seven(fs, time_per_beat, trip_spacing):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)    
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 35)
    sleep(time_per_beat)

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 40)
    sleep(time_per_beat * (1 - trip_spacing)) 

def s8_s_eight(fs, time_per_beat, trip_spacing):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)    
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 45)
    sleep(time_per_beat * (1 - trip_spacing)) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50)
    sleep(time_per_beat * (1 - trip_spacing)) 

#---------------------------------------- 8TH BASS DRUM BASS DRUM PHRASES ----------------------------
    
def s8_b_one(fs, time_per_beat, trip_spacing):   
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 50)
    sleep(time_per_beat * (1 - trip_spacing)) 

    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * (1 - trip_spacing))  

def s8_b_two(fs, time_per_beat, trip_spacing):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat) 
  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 50)
    sleep(time_per_beat * (1 - trip_spacing))  
 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)   
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * (1 - trip_spacing)) 

def s8_b_three(fs, time_per_beat, trip_spacing):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 35)    
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 40)
    sleep(time_per_beat * (1 - trip_spacing)) 

    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * (1 - trip_spacing)) 

def s8_b_four(fs, time_per_beat, trip_spacing):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 45)   
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 40)
    sleep(time_per_beat * (1 - trip_spacing))  

    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)   
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * (1 - trip_spacing)) 

def s8_b_five(fs, time_per_beat, trip_spacing):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)    
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50)
    sleep(time_per_beat * (1 - trip_spacing)) 

    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 60)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 50)
    sleep(time_per_beat * (1 - trip_spacing)) 
 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)   
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * (1 - trip_spacing)) 


def s8_b_six(fs, time_per_beat, trip_spacing):
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 45)
    sleep(time_per_beat * (1 - trip_spacing)) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 55)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * (1 - trip_spacing)) 


def s8_b_seven(fs, time_per_beat, trip_spacing):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 40)    
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat)

    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 45)
    sleep(time_per_beat * (1 - trip_spacing)) 
  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)   
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * (1 - trip_spacing)) 

def s8_b_eight(fs, time_per_beat, trip_spacing):  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 45)
    sleep(time_per_beat * (1 - trip_spacing)) 

    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 50)
    sleep(time_per_beat * (1 - trip_spacing)) 
 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)   
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * (1 - trip_spacing)) 


#-------------------------------8TH CRASH PHRASES ------------------------------------
    
def s8_crash_one(fs, time_per_beat, trip_spacing):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)    
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 55)
    sleep(time_per_beat * (1 - trip_spacing)) 

    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 70)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, CRASH_SIZ_V2, 115)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 50)
    sleep(time_per_beat * (1 - trip_spacing)) 

    sleep(time_per_beat) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)   
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * (1 - trip_spacing)) 

def s8_crash_two(fs, time_per_beat, trip_spacing):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)    
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50)
    sleep(time_per_beat * (1 - trip_spacing)) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 80)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, CRASH_SIZ_V2, 110)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 70)
    sleep(time_per_beat * (1 - trip_spacing)) 

    sleep(time_per_beat) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)   
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * trip_spacing)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * (1 - trip_spacing)) 




#--------------------------------- TRIP SNARE PHRASES -----------------------------------
def t8_s_one(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 40)  
    sleep(time_per_beat * 2/3)  

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15) 
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 40)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 55)
    sleep(time_per_beat * 1/3)  

def t8_s_two(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 45)  
    sleep(time_per_beat * 1/3)  
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50) 
    sleep(time_per_beat * 1/3) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 45)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 45)  
    sleep(time_per_beat * 1/3)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 55)
    sleep(time_per_beat * 1/3)  

def t8_s_three(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 45)  
    sleep(time_per_beat * 1/3)  
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50) 
    sleep(time_per_beat * 1/3) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 60)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * 2/3) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 60)
    sleep(time_per_beat * 1/3)  


def t8_s_four(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 45)  
    sleep(time_per_beat * 1/3)  
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 50) 
    sleep(time_per_beat * 1/3) 

    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 65)   
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * 1/3)  


#---------------------------------- TRIP BASS DRUM PHRASES ----------------------
def t8_b_one(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 40)  
    sleep(time_per_beat * 2/3)  
 
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 45)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 60)
    sleep(time_per_beat * 1/3)  

    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)   
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * 1/3) 

def t8_b_two(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 45)  
    sleep(time_per_beat * 1/3)  
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50) 
    sleep(time_per_beat * 1/3) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 45)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 45)  
    sleep(time_per_beat * 1/3)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 60)
    sleep(time_per_beat * 1/3)  

    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)   
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * 1/3) 

def t8_b_three(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 45)  
    sleep(time_per_beat * 1/3)  
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50) 
    sleep(time_per_beat * 1/3) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 60)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * 2/3)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * 1/3)  

def t8_b_four(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 45)  
    sleep(time_per_beat * 1/3)  
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50) 
    sleep(time_per_beat * 1/3) 

    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 60)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50) 
    sleep(time_per_beat * 2/3)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 60)
    sleep(time_per_beat * 1/3)  
 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)   
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * 1/3) 


#---------------- TRIP CRASH PHRASES ----------------------------
    
def t8_crash_one(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 55)  
    sleep(time_per_beat * 1/3)  
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 60) 
    sleep(time_per_beat * 1/3) 

    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 60)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 65)  
    sleep(time_per_beat * 1/3)  
    fs.noteon(DRUM_CHANNEL, CRASH_SIZ_V2, 110)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 80)
    sleep(time_per_beat * 1/3)  

    sleep(time_per_beat) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)   
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * 1/3) 

def t8_crash_two(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 90)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 45)  
    sleep(time_per_beat * 1/3)  
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 55) 
    sleep(time_per_beat * 1/3) 

    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 65)  
    sleep(time_per_beat * 1/3)  
    fs.noteon(DRUM_CHANNEL, CRASH_SIZ_V2, 110)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 80)
    sleep(time_per_beat * 1/3)  

    sleep(time_per_beat) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 15)   
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 105)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 50)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 95)
    sleep(time_per_beat * 1/3) 


#------------------------------- MISC ----------------------------

def phrase_two(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 25)    
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 30)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 60)
    sleep(time_per_beat * 1/3)

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 90) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 127)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 30)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 60)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 1/3)  


def phrase_three(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 25)    
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 30)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 60)
    sleep(time_per_beat * 1/3)

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 110) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 127)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 1/3)

def phrase_four(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 85)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    sleep(time_per_beat) 
    
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 127)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 2/3)    
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 85) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 1/3)  

def phrase_five(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 25)    
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    sleep(time_per_beat)

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 25)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 127)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 70)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 1/3)  

def phrase_six(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 80)  
    sleep(time_per_beat * 1/3)


    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 127)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 1/3) 

def phrase_seven(fs, time_per_beat): 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 40)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 70)
    sleep(time_per_beat * 1/3)

    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 40) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 127)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 40)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 70)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 1/3)  

def phrase_eight(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 40)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 45) 
    sleep(time_per_beat * 2/3)

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 40) 
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 45) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 127)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 90) 
    sleep(time_per_beat * 1/3)  

#-------------------  SLOW PHRASES --------------------------
    

def phrase_nine(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 40)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 40)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50)
    sleep(time_per_beat * 1/3)

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 40) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 75)
    sleep(time_per_beat * 1/3)

def phrase_ten(fs, time_per_beat): 
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 40)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 40)
    sleep(time_per_beat * 1/3) 
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50)
    sleep(time_per_beat * 1/3) 

    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 70) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 75)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 90)
    sleep(time_per_beat * 1/3)  

def phrase_eleven(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 40)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 30)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 60)
    sleep(time_per_beat * 1/3)

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 90) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 30)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 90)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 75)
    sleep(time_per_beat * 1/3)  

def phrase_twelve(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 40)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 50)    
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 40)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 90) 
    sleep(time_per_beat * 1/3)


    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 30)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 45)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 75)
    sleep(time_per_beat * 1/3)  

#--------------------- FAST PHRASES -----------------------------
    
def f_swing_pattern(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 30)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    sleep(time_per_beat) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 30) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 127)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 7/12)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 5/12) 

def f_phrase_one(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 30)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    sleep(time_per_beat * 7/12)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 60)
    sleep(time_per_beat * 5/12) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 110) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 127)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 7/12)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 5/12)  

def f_phrase_three(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 30)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 30)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 60)
    sleep(time_per_beat * 1/3)

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 110) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 127)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 7/12)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 5/12)

def f_phrase_five(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 30)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    sleep(time_per_beat)

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 30)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 127)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 7/12)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 70)
    sleep(time_per_beat * 5/12)  

def f_phrase_six(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    sleep(time_per_beat * 7/12)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 80)  
    sleep(time_per_beat * 5/12)


    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 127)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 7/12)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 5/12) 
    
def f_phrase_thirteen(fs, time_per_beat): 
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 30)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 7/12)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 60)
    sleep(time_per_beat * 5/12) 

    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 70) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 7/12)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 75)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 90)
    sleep(time_per_beat * 5/12) 

def f_phrase_fourteen(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 30)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 7/12)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 60)
    sleep(time_per_beat * 5/12) 

    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 80) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 30)
    sleep(time_per_beat * 7/12)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 75)
    sleep(time_per_beat * 5/12)  

def f_phrase_fifteen(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 30)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 70)
    sleep(time_per_beat * 7/12)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 65)
    sleep(time_per_beat * 5/12)

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 30) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 7/12)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 75)
    sleep(time_per_beat * 5/12)

def f_phrase_sixteen(fs, time_per_beat): 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 7/12)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 80)
    sleep(time_per_beat * 5/12) 

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 100) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 7/12)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 75)
    sleep(time_per_beat * 5/12) 

#------------------------ LEGATO/BIG PHRASES FOR CRASHING/ACCENTUATION --------------
    
def d_phrase_five(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 40)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 70)
    sleep(time_per_beat * 1/3)

    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 90) 
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 70)
    fs.noteon(DRUM_CHANNEL, CRASH_SIZ_V2, 100)
    sleep(time_per_beat)   

    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 60)
    sleep(time_per_beat * 1/3) 

    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 80) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 127)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 1/3)  


    
def d_phrase_eight(fs, time_per_beat):
    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 40)  
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 115)
    sleep(time_per_beat * 1/3)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 45) 
    sleep(time_per_beat * 2/3)

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 40) 
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 45) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 127)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, CRASH_SIZ_V2, 100)
    fs.noteon(DRUM_CHANNEL, SNARE_DRUM, 90) 
    sleep(time_per_beat * 4/3)

    fs.noteon(DRUM_CHANNEL, BASS_DRUM, 40) 
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 127)
    fs.noteon(DRUM_CHANNEL, HI_HAT_CLOSED, 80)
    sleep(time_per_beat * 2/3)
    fs.noteon(DRUM_CHANNEL, RIDE_CYMBAL, 100)
    sleep(time_per_beat * 1/3)  


