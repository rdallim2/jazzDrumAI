        ...
        try:
            ...
            for i in range(pygame.midi.get_count()):
                ...
                if is_input:
                    ...
                    print(f"  Device {i}: {name} (Input: {is_input})")
            ...
            input_device_id = input_devices[3]  # Use first available input device
            print(f"Using MIDI input device {input_device_id}")
            ...
            try:
                ...
                try:
                    print("Listening for Keyboard MIDI input...")
                        ...
                        if midi_input.poll():
                            ...
                            for event in midi_events:
                                ...
                                if status == 0x90 and data[2] > 0:  # Note on with velocity > 0
                                    ...
                                    if fs is not None and 0 <= note <= 127:
                                        fs.noteon(10, note, velocity)  # Use channel 10 for user piano
                                
                                # Note off event (either note off status or note on with velocity 0)
                    ...
                    # Always close the MIDI input
                    midi_input.close()
                    print("MIDI input closed")
                    
            except Exception as e:
                print(f"Error opening MIDI device {input_device_id}: {e}")
