...
class DrumMachineGUI(BoxLayout):
    ...
    def update_chord_display(self, bar_index):
        ...
        if 0 <= current_index < len(self.chord_buttons):
            self.chord_buttons[current_index].background_color = (0.4, 0.5, 0.8, 1)  # Brighter highlighted color for better contrast

    def run_bass_wrapper(self, time_per_beat, bass_channel=9):
        try:
            # Reset the global bar_count for bass
            global bar_count
            bar_count = 0
            self.current_bar = 0
            ...
            while not stop_event.is_set():
                try:
                    # Update the UI to show current chord
                    Clock.schedule_once(lambda dt: self.update_chord_display(bar_count), 0)
                    
                    # Set the sync event at the start of each bar
                    instrument_sync.set()
                    
                    # Play one bar of bass
                    play_bar(fs, time_per_beat, channel=bass_channel)
                    
                    # Update the current bar for chord display
                    self.current_bar = bar_count
                    
                    # Clear the sync event after the bar
                    instrument_sync.clear()
