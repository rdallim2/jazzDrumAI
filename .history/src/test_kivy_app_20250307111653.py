...
class DrumMachineGUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        self.mode_spinner = Spinner(
            text='Select Mode',
            values=('Drums Only', 'Piano with Drums', 'Piano and Bass with Drums', 'Bass with Drums', 'Guitar/Bass/Drums'),
            size_hint=(None, None),
            size=(300, 50),
    ...
    def initialize_audio(self):
        try:
            ...
            guitar_sfid = fs.sfload("Electric_Jazz_Arc_htop_Guitar.sf2")
            fs.program_select(8, guitar_sfid, 0, 26) #0 channel, 26 bank for jazz guitar
    ...
    def run_guitar_wrapper(self, time_per_beat, guitar_channel=8):
        try:
            while not self.stop_event.is_set():
                # Simple bass pattern
                if not self.stop_event.is_set():
                    guitar_comp(fs, time_per_beat, guitar_channel)          
        except Exception as e:
            print(f"Error in bass thread: {e}")
    ...
    def start_performance(self, instance):
        ...
        try:
            ...
            if players == 1:
                ...
                guitar_thread = threading.Thread(
                    target=self.run_guitar_wrapper,
                    args=(time_per_beat, 9)
                )
                bass_thread = threading.Thread(
                    target=self.run_bass_wrapper,
                ...
                drum_thread.daemon = True
                guitar_thread.daemon = True
                bass_thread.daemon = True
                drum_thread.start()
                guitar_thread.start()
                bass_thread.start()
                self.running_threads.extend([drum_thread, guitar_thread, bass_thread])
