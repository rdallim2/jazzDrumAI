    ...
    def run_bass_wrapper(self, time_per_beat, bass_channel=1):
        try:
            while not self.stop_event.is_set():
                ...
                bar_start = time.time()
                note_count = 0
                pattern = get_current_bass_pattern()  # Get the current pattern
                
                for note in pattern:
                    fs.noteon(bass_channel, note, 80)
                    time.sleep(time_per_beat * 0.95)
                    fs.noteoff(bass_channel, note)
                    time.sleep(time_per_beat * 0.05)
                    
                    note_count += 1
                    if note_count % 2 == 0:  # Every two notes
                        sync.wait_for_beat()  # Wait for drum signal
    ...
    def run_piano_wrapper(self, time_per_beat, tempo, piano_channel=0):
        try:
            while not self.stop_event.is_set():
                ...
                chord_voicing = get_current_piano_chord()
                play_piano_comp_with_sync(fs, time_per_beat, tempo, piano_channel, chord_voicing, sync)
