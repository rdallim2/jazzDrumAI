...
def play_bar(fs, time_per_beat, channel=9):
    ...
    expected_bar_duration = BEATS_PER_BAR * time_per_beat 
    ...
    while True:  # Loop indefinitely for background bass
        ...
        # Adjust tempo to compensate for timing error
        time_per_beat -= timing_error / (BEATS_PER_BAR * total_bars)
        time_per_beat = max(time_per_beat, 0.01)  # Prevent time from becoming negative
