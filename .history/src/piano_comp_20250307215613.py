...
def play_piano_comp_with_sync(fs, time_per_beat, tempo, channel, chord_voicing, sync):
    """
    Plays a piano comp pattern with synchronization points on 2nd and 4th notes.
    """
    # Play first note
    ...
    # Play second note (with sync)
    sync.wait_for_beat()
    for note in chord_voicing:
        fs.noteon(channel, note, 75)
    time.sleep(time_per_beat * 0.95)
    for note in chord_voicing:
        fs.noteoff(channel, note)
    time.sleep(time_per_beat * 0.05)
...
def get_current_piano_chord():
    """Helper function to get the current chord voicing based on bar position"""
    bar_count = 0
    total_bars = 12
    current_bar = bar_count % total_bars
    ...
    return random.choice(chord_voicings)
