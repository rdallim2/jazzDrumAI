...
class DrumMachineGUI(BoxLayout):
    ...
    def run_drums_wrapper(self, time_per_beat, tempo, players):
        try:
            print("Starting drum wrapper")
            while not self.stop_event.is_set():
                # Just do a simple swing pattern for testing
                print("Playing pattern")
                # Use channel 4 for drums
                fs.noteon(4, BASS_DRUM, 15)  
                fs.noteon(4, RIDE_CYMBAL, 90)
                sleep(time_per_beat) 

                fs.noteon(4, BASS_DRUM, 15)   
                fs.noteon(4, RIDE_CYMBAL, 105)
                fs.noteon(4, HI_HAT_CLOSED, 70)
                sleep(time_per_beat * 2/3)
                fs.noteon(4, RIDE_CYMBAL, 95)
                sleep(time_per_beat * 1/3)
                
                if self.stop_event.is_set():
                    print("Stop event detected")
                    break
            print("Exited drum loop")
        except Exception as e:
    ...
    def run_bass_wrapper(self, time_per_beat, output_device):
        try:
            while not self.stop_event.is_set():
                # Simple bass pattern
                if not self.stop_event.is_set():
                    fs.noteon(0, 36, 100)  # Bass note on
                    time.sleep(time_per_beat * 0.5)
                    fs.noteoff(0, 36)  # Bass note off
                    time.sleep(time_per_beat * 0.5)
        except Exception as e:
